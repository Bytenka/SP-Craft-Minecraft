...
public final class CraftDB {
	...
	public static Item[][] getCraftFromItem(Item item) {
		for (int i = 0; i < craftDB.length; i++) {
			InternalCraft craft = craftDB[i].get(item);
			if (craft != null)
				return craft.recipe.getItemTab();
		}
		return null;
	}
	...
	private static int stringSizeToIndex(String size) {
		...
		case "33":
			return 8;
		default:
			throw new RuntimeException("No matching index for size " + size);
		}
	}
